...
void dlgMain::OnCheckVersionResponse(wxWebRequestEvent& evt)
{
    if (evt.GetState() == wxWebRequest::State_Completed)
    {
        ...
        Json::Value root;
        std::string errors;
        if (!reader->parse(utf8.data(), utf8.data() + utf8.length(), &root,
               &errors) ||
               !root.isObject())
        {
            InfoBar->ShowMessage("Unable to check for updates.");
            wxLogWarning("Odalaunch tried to parse malformed JSON when checking for updates.");
            return;
        }
        ...
        {
            InfoBar->ShowMessage("Unable to check for updates.");
            wxLogWarning("Odalaunch couldn't find a new version to compare against when checking for updates.");
            return;
        }
    ...
    {
        InfoBar->ShowMessage("Unable to check for updates.");
        wxLogWarning("Odalaunch could not connect to %s to check for updates.", evt.GetResponse().GetURL().c_str());
    }
}
#endif
