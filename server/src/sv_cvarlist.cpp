...
CVAR(sv_latency, "80", "Latency simulation", cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE) //number of miliseconds to delay packet send, this will cause ping to be ~ sv_latency + network latency
#endif
...
CVAR_RANGE(		sv_maxcorpses, "200", "Maximum corpses to appear on map",
				cvartype_t::WORD, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 0.0f, 65536.0f)

CVAR(			sv_clientcount,	"0", "Set to the number of connected players (for scripting)",
				cvartype_t::BYTE, CVAR_NOSET | CVAR_NOENABLEDISABLE)

CVAR_RANGE_FUNC_DECL(sv_maxclients, "4", "Maximum clients that can connect to a server",
				cvartype_t::BYTE, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 2.0f, 255.0f)
...
CVAR_RANGE_FUNC_DECL(sv_maxplayersperteam, "3", "Maximum number of players that can be on a team (0 means unlimited)",
				cvartype_t::BYTE, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f, 255.0f)

CVAR_FUNC_DECL(	join_password, "", "Clients can connect if they have this password",
				cvartype_t::STRING, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE)
...
CVAR_RANGE_FUNC_DECL (net_maxthreads, "0", "Number of threads to use for client messaging (0 to auto-select based on core count)",
                      cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 0.0f, 256.0)
...
CVAR_RANGE(		sv_flooddelay, "1.5", "Chat flood protection time (in seconds)",
				cvartype_t::FLOAT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 0.0f, 10.0f)

CVAR_RANGE_FUNC_DECL(sv_maxrate, "800", "Forces clients to be on or below this rate",
				cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 7.0f, 100000.0f)
...
CVAR_RANGE(		sv_upnp_discovertimeout, "2000", "UPnP Router discovery timeout",
				cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 500.0f, 10000.0f)
...
CVAR_RANGE(		sv_teamsinplay, "2", "Teams that are enabled",
				cvartype_t::BYTE, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 2.0f, 3.0f)
...
CVAR_RANGE(		sv_banfile_reload, "0", "Number of seconds to wait between automatically loading the banlist.",
				cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 0.0f, 86400.0f)
...
CVAR_RANGE(		sv_vote_majority, "0.5", "Ratio of yes votes needed for vote to pass.",
				cvartype_t::FLOAT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 0.0f, 1.0f)
...
CVAR_RANGE(		sv_vote_timelimit, "30", "Amount of time a vote takes in seconds.",
				cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 0.0f, 120.0f)

CVAR_RANGE(		sv_vote_timeout, "60", "Timeout between votes in seconds.",
				cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE, 0.0f, 600.0f)
