...
CVAR_FUNC_IMPL (sv_maxclients)
{
	...
	while (it != players.end())
	{
		if (count <= 0)
		{
			SVC_Print(it->client.reliablebuf, PRINT_CHAT,
...
void SV_UpdateSector(client_t* cl, int sectornum)
{
	...
	{
		SVC_UpdateSector(cl->reliablebuf, *sector);
	}
}
...
void SV_SendMovingSectorUpdate(player_t &player, sector_t *sector)
{
	...
	SVC_MovingSector(*netbuf, *sector);
}
...
bool SV_CheckClientVersion(client_t *cl, Players::iterator it)
{
	...
	if (!AllowConnect)
	{
		...
		SVC_Print(cl->reliablebuf, PRINT_HIGH, FormattedString.str());
...
void SV_ConnectClient()
{
	...
	if (strlen(join_password.cstring()) && MD5SUM(join_password.cstring()) != passhash)
	{
		...
		SVC_Print(cl->reliablebuf, PRINT_HIGH,
...
void STACK_ARGS SV_BroadcastPrintf(int printlevel, const char* format, ...)
{
	...
	for (Players::iterator it = players.begin(); it != players.end(); ++it)
	{
		...
		SVC_Print(cl->reliablebuf, static_cast<printlevel_t>(printlevel), string);
	}
}
...
void STACK_ARGS SV_SpectatorPrintf(int level, const char *fmt, ...)
{
	...
	for (Players::iterator it = players.begin(); it != players.end(); ++it)
	{
		...
		if (spectator)
		{
			SVC_Print(cl->reliablebuf, static_cast<printlevel_t>(level), string);
		}
	}
}
...
void SV_RConPassword (player_t &player)
{
	...
	else
	{
		...
		SVC_Print(cl->reliablebuf, PRINT_HIGH, "Bad password\n");
	}
}
...
void SV_WantWad(player_t &player)
{
	...
	if(!sv_waddownload)
	{
		...
		SVC_Print(cl->reliablebuf, PRINT_HIGH, "Server: Downloading is disabled\n");
	...
	if (i == wadfiles.size())
	{
		SVC_Print(cl->reliablebuf, PRINT_HIGH, "Server: Bad wad request\n");
		SV_DropClient(player);
		return;
	}
	...
	if (W_IsIWAD(wadfiles[i]))
	{
		...
		SVC_Print(cl->reliablebuf, PRINT_HIGH, message);
