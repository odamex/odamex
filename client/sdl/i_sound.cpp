...
static byte *expand_sound_data(byte *data, int samplerate, int length)
{
   ...
   if (samplerate == 11025)
   {
      ...
      expanded = (byte *)Z_Malloc(length * 4, PU_STATIC, NULL);
   ...
   {
      expanded = (byte *)Z_Malloc(length * 2, PU_STATIC, NULL);
   ...
   else
   {
      ...
      expanded = (byte *)Z_Malloc(expanded_length * 2, PU_STATIC, NULL);
...
static void getsfx (struct sfxinfo_struct *sfx)
{
	...
	data = (byte *)W_CacheLumpNum(sfx->lumpnum, PU_STATIC);
	...
	chunk->allocated = 1;
	chunk->abuf = (Uint8 *)expand_sound_data(data + 8, samplerate, length);
	chunk->alen = (length * 2) * (22050 / samplerate);
