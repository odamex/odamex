...
// (it also declares g_spawnatdeathspot here).
...
// MERGE ALERT
// this conflicts when merging stable into protobreak - take protobreak's version
// (RespawnText and DrawRespawnText there include the death spot prompts).
/**
...
std::vector<std::string> RespawnText()
{
	...
	lines.push_back(fmt::sprintf("Press %s to respawn", KeyPrompt("+use")));
