...
//		return;
	...
	if (!r_columnmethod || colfunc == fuzzcolfunc) {
		...
		for (dc_x = x1; dc_x <= x2; dc_x++, frac += xiscale)
		{
			...
			R_DrawMaskedColumn ((column_t *)((byte *)patch + SWAP_DWORD(patch->columnofs[texturecolumn])));
		}
	} else {
		...
		if (x1 < x2) {
			...
			if (dc_x & 1) {
				R_DrawMaskedColumn ((column_t *)((byte *)patch + SWAP_DWORD(patch->columnofs[frac>>FRACBITS])));
				dc_x++;
				frac += xiscale;
			}
			...
			if (dc_x & 2) {
				if (dc_x < x2 - 1) {
					rt_initcols();
					R_DrawMaskedColumnHoriz ((column_t *)((byte *)patch + SWAP_DWORD(patch->columnofs[frac>>FRACBITS])));
					dc_x++;
					frac += xiscale;
					R_DrawMaskedColumnHoriz ((column_t *)((byte *)patch + SWAP_DWORD(patch->columnofs[frac>>FRACBITS])));
