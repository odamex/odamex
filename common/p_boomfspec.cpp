...
void P_PlayerInCompatibleSector(player_t* player)
{
	...
	if (sector->special < 32) // regular sector specials
	{
		...
		{
		case 5:
			P_ApplySectorDamage(player, 10, false);
			break;
		case 7:
			P_ApplySectorDamage(player, 5, false);
			break;
		case 16:
		case 4:
			P_ApplySectorDamage(player, 20, true);
			break;
		case 9:
...
void P_SpawnCompatibleScroller(line_t* l, int i)
{
	...
	case 250: // scroll effect ceiling
		for (s = -1; (s = P_FindSectorFromLineTag(l->id, s)) >= 0;)
                                        ...
                                        bool bossaction)
{
	...
	case 133:
		...
		if (EV_DoDoor(DDoor::doorOpen, line, thing, line->id,
		              SPEED(D_FAST), TICS(0), BCard | CardIsSkull))
		{
			result.lineexecuted = true;
			result.switchchanged = true;
		}
...
void P_ShootCompatibleSpecialLine(AActor* thing, line_t* line)
{
	...
	case 24:
		...
		if (EV_DoFloor(line, raiseFloor) || demoplayback)
			P_ChangeSwitchTexture(line, 0);
		break;
	...
	case 47:
		...
		if (EV_DoPlat(line, raiseToNearestAndChange, 0) || demoplayback)
			P_ChangeSwitchTexture(line, 0);
		break;
