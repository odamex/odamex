...
BOOL EV_CompatibleTeleport(int tag, line_t* line, int side, AActor* thing, int flags)
{
	...
	// [AM] Use modern ZDoom teleport destination selection.
	m = SelectTeleDest(thing->tid, tag);
	if (m == NULL)
		return false;
	...
	else if (m->type == MT_TELEPORTMAN && (gamemission < pack_tnt || gamemission == chex))
	{
		thing->z = thing->floorz;
	}
	thing->prevz = thing->z;
	...
	// spawn teleport fog at source and destination
	// [RK] but account for nosourcefog and nostop fog arguments
	// [Blair] Modify this slightly to use teleport flags
	if (serverside && !(player && player->spectator))
	{
		if (flags & TELF_SOURCEFOG)
		{
			S_Sound(new AActor(oldx, oldy, oldz, MT_TFOG), CHAN_VOICE, "misc/teleport", 1,
			        ATTN_NORM);
		}

		if (flags & TELF_DESTFOG)
		{
			S_Sound(new AActor(m->x + 20 * finecosine[m->angle >> ANGLETOFINESHIFT],
