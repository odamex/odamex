                      ...
                      unsigned int flags2)
{
	...
	if (os.getToken()[0] == '$')
	{
		...
		// prefixed by a $.
		const OString& s = GStrings(OStringToUpper(os.getToken()).c_str() + 1);
		if (s.empty())
		{
                          ...
                          unsigned int flags2)
{
	...
	if (musicname[0] == '$')
	{
		// It is possible to pass a DeHackEd string
		// prefixed by a $.
		const OString& s = GStrings(OStringToUpper(musicname.c_str() + 1));
		if (s.empty())
		{
			os.error("Unknown lookup string \"{}\".", os.getToken());
		}
                          ...
                          unsigned int flags2)
{
	...
	if (newStyleMapInfo)
	{
		if (os.compareTokenNoCase("lookup"))
		{
			...
			os.mustScan();
			const OString& s = GStrings(OStringToUpper(os.getToken()));
			if (s.empty())
			{
				os.error("Unknown lookup string \"{}\".", os.getToken());
			}
