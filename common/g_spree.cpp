...
void SpreeManager::expireOldSprees()
{
	...
	// C++17 equivalent of std::erase_if - erasing through the returned iterator
	// keeps the traversal valid.
	for (auto it = spreeRecord.begin(); it != spreeRecord.end();)
	{
		// Spree happened in the future, indicating we're in a rewinded demo
		// Remove it
		if (it->second.spreeStartTic > ::gametic)
		{
			it = spreeRecord.erase(it);
		}
		else
		{
			++it;
...
void SpreeManager::serialize(FArchive& arc)
{
	if (arc.IsStoring())
	{
		...
		{
			arc << playerId << record.playerName.c_str() << record.spreeLevel
			    << record.spreeStartTic << record.stillDominating
			    << record.spree.spreeText.c_str()
			    << record.spree.spreeBroadcastText.c_str()
			    << record.spree.gameSfxToken.c_str()
			    << static_cast<int32_t>(record.spree.color);
		}

		arc << spreeBreaker.spreeEndedName.c_str() << spreeBreaker.spreeEndedPlayerId
		    << static_cast<int32_t>(spreeBreaker.spreeEndedTeam)
		    << spreeBreaker.spreeEnderName.c_str() << spreeBreaker.spreeEnderPlayerId
		    << static_cast<int32_t>(spreeBreaker.spreeEnderTeam)
		    << spreeBreaker.spreeEndedBroadcastText.c_str()
		    << spreeBreaker.spreeEnded.c_str()
		    << static_cast<int32_t>(spreeBreaker.spreeEndedColor)
		    << spreeBreaker.spreeEnderMonster
		    << spreeBreaker.endedPoints << spreeBreaker.spreeEndedTic
		    << spreeBreaker.spreeEndedLevel
		    << static_cast<int32_t>(spreeBreaker.spreeEndedType);
	}
	else
	{
		spreeRecord.clear();
		...
		for (uint32_t i = 0; i < count; i++)
		{
			...
			arc >> record.playerId >> record.playerName >> record.spreeLevel >>
			    ...
			    record.spree.spreeBroadcastText >> record.spree.gameSfxToken >> color;
			record.spree.color = static_cast<EColorRange>(color);
		...
		arc >> spreeBreaker.spreeEndedName >> spreeBreaker.spreeEndedPlayerId >>
		    ...
		    endedColor >> spreeBreaker.spreeEnderMonster >>
		    spreeBreaker.endedPoints >> spreeBreaker.spreeEndedTic >>
		    spreeBreaker.spreeEndedLevel >> endedType;
		spreeBreaker.spreeEndedTeam = static_cast<team_t>(endedTeam);
		spreeBreaker.spreeEnderTeam = static_cast<team_t>(enderTeam);
		spreeBreaker.spreeEndedColor = static_cast<EColorRange>(endedColor);
		spreeBreaker.spreeEndedType = static_cast<SpreeBreakerType>(endedType);
	}
}
