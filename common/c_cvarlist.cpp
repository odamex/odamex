...
CVAR(				sv_friendlyfire, "1", "When set, players can injure others on the same team, " \
					...
					cvartype_t::BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO)
...
CVAR_RANGE(			sv_scorelimit, "5", "Game ends when team score is reached in Teamplay/CTF",
					cvartype_t::BYTE, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 255.0f)

CVAR(				sv_teamspawns, "1", "When disabled, treat team spawns like normal deathmatch " \
					"spawns in Teamplay/CTF",
					cvartype_t::BOOL, CVAR_SERVERARCHIVE | CVAR_LATCH | CVAR_SERVERINFO)
...
CVAR_RANGE(			sv_weapondamage, "1.0", "Amount to multiply player weapon damage by",
					cvartype_t::FLOAT, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE,
					0.0f, 100.0f)

CVAR_FUNC_DECL(		sv_forcewater, "0", "Makes water more realistic (boom maps at the moment)",
					cvartype_t::BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE)
...
CVAR_RANGE(			sv_fraglimit,     "0", "Sets the amount of frags a player can accumulate before the game ends",
					cvartype_t::WORD, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 65536.0f)
...
CVAR(				sv_itemsrespawn, "0", "Items will respawn after a fixed period, see sv_itemrespawntime",
					cvartype_t::BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH)
...
CVAR_RANGE(			sv_itemrespawntime, "30", "If sv_itemsrespawn is set, items will respawn after this " \
					"time (in seconds)",
					cvartype_t::WORD, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 500.0f)
...
CVAR_RANGE(			sv_skill,"3", "Sets the skill level, values are:\n" \
					...
					"// 4 - Ultra-Violence\n" \
					"// 5 - Nightmare",
					cvartype_t::BYTE, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f, 7.0f)

CVAR_RANGE(sv_timelimit, "0", "Sets the time limit for the game to end (in minutes)",
           cvartype_t::FLOAT, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE,
           0.0f, 1440.0f)

CVAR_RANGE_FUNC_DECL(sv_intermissionlimit, "10", "Sets the time limit for the intermission to end (in seconds).",
					cvartype_t::WORD, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 60.0f)
...
CVAR_RANGE(			sv_maxunlagtime, "1.0", "Cap the maximum time allowed for player reconciliation (in seconds)",
					cvartype_t::FLOAT, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 1.0f)
...
CVAR(				sv_allowredscreen, "1","Allow clients to adjust amount of red pain screen intensity",
					cvartype_t::BOOL, CVAR_SERVERINFO | CVAR_SERVERARCHIVE)
...
CVAR_FUNC_DECL(		sv_allowwidescreen, "1", "Allow clients to use true widescreen",
					cvartype_t::BOOL, CVAR_SERVERINFO | CVAR_SERVERARCHIVE | CVAR_LATCH)
...
CVAR_RANGE(			sv_forcerespawntime, "30", "Force a player to respawn after a set amount of time (in seconds)",
					cvartype_t::WORD, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 32768.0f)

CVAR_RANGE(			sv_spawndelaytime, "0.0", "Force a player to wait a period (in seconds) before respawning",
					cvartype_t::FLOAT, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 32768.0f)
...
CVAR(				sv_hostname, "Untitled Odamex Server", "Server name to appear on masters, clients and launchers",
					cvartype_t::STRING, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE | CVAR_SERVERINFO)
...
CVAR(sv_downloadsites, "",
     ...
     cvartype_t::STRING, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE)
...
CVAR(g_ctf_notouchreturn, "0",
     ...
     cvartype_t::BOOL, CVAR_SERVERARCHIVE | CVAR_LATCH)
...
CVAR(g_roundlimit, "0",
     "Number of total rounds the game can go on for before the game ends", cvartype_t::INT,
...
CVAR(g_lives_jointimer, "0",
     ...
     cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE)
...
CVAR(g_preroundtime, "5", "Amount of time before a round where you can't shoot",
     cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE)

CVAR(g_preroundreset, "0", "After preround is over, reset the map one last time.",
     cvartype_t::INT, CVAR_SERVERARCHIVE)
...
CVAR_RANGE(g_thingfilter, "0", "Removes some things from the map. Values are:\n" \
           ...
           cvartype_t::BYTE, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE | CVAR_LATCH,
           -1.0f, 3.0f)

CVAR(g_resetinvonexit, "0",
     "Always reset players to their starting inventory on level exit", cvartype_t::BOOL,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE)

CVAR(g_horde_waves, "5", "Number of horde waves per map", cvartype_t::INT,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE)
...
CVAR(g_horde_goalhp, "8.0", "Goal health multiplier for a given round", cvartype_t::FLOAT,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE)

CVAR_RANGE(g_horde_spawnempty_min, "1",
           "Minimum number of seconds it takes to spawn a monster in an empty horde map",
           cvartype_t::INT, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 1,
           60)
...
CVAR_RANGE(g_horde_extralife, "0.0", "Chance to spawn an `extra life powerup` in Horde.\n" \
     ...
     cvartype_t::FLOAT, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f, 4.0f)
...
CVAR(sv_warmup, "0", "Enable a 'warmup mode' before the match starts.", cvartype_t::BOOL,
...
CVAR_RANGE(sv_warmup_autostart, "1.0",
           ...
           cvartype_t::FLOAT, CVAR_SERVERARCHIVE | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f,
           1.0f)
CVAR_RANGE(sv_countdown, "5",
           "Number of seconds to wait before starting the game from "
           "warmup or restarting the game.",
           cvartype_t::BYTE, CVAR_SERVERARCHIVE | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f,
           60.0f)
...
// ---------------------------------
	...
	CVAR(			co_realactorheight, "0", "Enable/Disable infinitely tall actors",
					cvartype_t::BOOL, CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_LATCH)

	CVAR(			co_nosilentspawns, "0", "Turns off the west-facing silent spawns vanilla bug",
					cvartype_t::BOOL, CVAR_ARCHIVE | CVAR_SERVERINFO)
	...
	CVAR_RANGE(co_friend_distance, "128",
					...
					cvartype_t::INT, CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 2048.0f)
	...
	CVAR_RANGE(co_friend_playerhelpers, "0",
					...
					cvartype_t::INT, CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f, 64.0f)
	...
	CVAR(			co_friend_helpertype, "",
					...
					cvartype_t::STRING, CVAR_ARCHIVE | CVAR_NOENABLEDISABLE)
	...
	CVAR(			co_globalsound, "0", "Make pickup sounds global", cvartype_t::BOOL, CVAR_ARCHIVE | CVAR_SERVERINFO)
...
CVAR_RANGE_FUNC_DECL(sv_gravity, "800", "Gravity of the environment",
					cvartype_t::WORD, CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE,
					0.0f, 32768.0f)

CVAR_RANGE_FUNC_DECL(sv_aircontrol, "0.00390625", "How much control the player has in the air",
					cvartype_t::FLOAT, CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE,
					0.0f, 32768.0f)
...
CVAR(				port, "0", "Display currently used network port number",
					cvartype_t::INT, CVAR_NOSET | CVAR_NOENABLEDISABLE)

CVAR_RANGE(			chase_height, "-8", "Height of chase camera",
					cvartype_t::WORD, CVAR_CLIENTARCHIVE | CVAR_NOENABLEDISABLE, -32768.0f, 32768.0f)
