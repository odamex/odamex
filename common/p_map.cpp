...
bool PIT_CheckThing (AActor& thing)
{
	...
	if (tmthing && !(tmthing->player && thing.player) &&
...
bool P_IsBridgeMobj(const AActor& thing)
{
	return (thing.type == MT_BRIDGE ||
...
bool P_ShootLine(line_t& li, const fixed_t frac)
{
	...
	const bool skyceiling1 = R_ResourceIdIsSkyFlat(sec1->ceiling_res_id);
	const bool skyceiling2 = sec2 != nullptr && R_ResourceIdIsSkyFlat(sec2->ceiling_res_id);
                           ...
                           const int bombmod)
{
	...
	const fixed_t dx = abs(thing.x - bombspot->x);
	const fixed_t dy = abs(thing.y - bombspot->y);
	auto len = static_cast<float>(std::max(dx, dy));
	const auto boxradius = static_cast<float>(thing.radius);
	...
	if (points > 0.0f && P_CheckSight(&thing, bombspot))
	{
		...
		const int damage = static_cast<int>(points);
...
void P_CopySector(sector_t *dest, sector_t *src)
{
	...
	dest->floor_res_id			= src->floor_res_id;
	dest->ceiling_res_id		= src->ceiling_res_id;
