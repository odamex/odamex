...
void P_InitMobjNameMap()
{
	...
	MapMobj(MT_CHAINGUN, "Chaingun", wp_chaingun);
	...
	MapMobj(MT_GRENADE, "Grenade");
	MapMobj(MT_BFG10KSHOT, "BFG10kShot");
	MapMobj(MT_TURBOSPHERE, "Turbosphere", pw_turbosphere);
	MapMobj(MT_TIMEFREEZER, "TimeFreezeSphere", pw_freezer);
	MapMobj(MT_INVISIBILITY, "InvisibilitySphere", pw_translucency);
	MapMobj(MT_DOOMSPHERE, "Doomsphere", pw_doomsphere);
	MapMobj(MT_RUNESTRENGTH, "RuneStrength");
	MapMobj(MT_RUNERAGE, "RuneRage");
	MapMobj(MT_RUNEDRAIN, "RuneDrain");
	MapMobj(MT_RUNERESISTANCE, "RuneResistance");
	MapMobj(MT_RUNEREGENERATION, "RuneRegeneration");
	MapMobj(MT_RUNEPROSPERITY, "RuneProsperity");
	MapMobj(MT_RUNEHIGHJUMP, "RuneHighJump");
	MapMobj(MT_RUNEHASTE, "RuneHaste");
	MapMobj(MT_RUNESPREAD, "RuneSpread");
	MapMobj(MT_RUNEREFLECTION, "RuneReflection");
